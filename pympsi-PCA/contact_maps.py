...
def plot_maps(maps, groups, titles, name, outpath):
    ...
    for idx, ax in enumerate(axes):
        ...
        ax.set_yticklabels([f"{r.resname}-{r.resid}" for r in grp1], fontsize=18)
        ...
        for m, l in zip(mids1, labs1):
            ax.text(1.01, m, l, ha='left', va='center', rotation=90, 
                    fontsize=18, transform=ax.get_yaxis_transform())
        ...
        ax.set_title(titles[idx], pad=18)

    cbar = fig.colorbar(im, ax=axes, orientation='horizontal',
                        fraction=0.05, pad=0.02)
    cbar.set_label("Contact probability")
    fig.suptitle(f"Contact Maps", fontsize=18)
