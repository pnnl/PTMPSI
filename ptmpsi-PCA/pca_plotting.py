...
def plot_cluster_feature_ranges(cluster_feature_ranges, feat_names, system_name, outdir="figures_pca"):
    ...
    for i, feat in enumerate(feat_names):
        ...
        ax.set_ylabel(unit_str, fontsize=14)
...
def plot_top_pcs(pca_data, explained_variance, system_name, outdir="figures_pca"):
    ...
    ax_big.set_title(f"PC1 ({explained_variance[0]*100:.1f}% var)", fontsize=14)
    ax_big.set_xlabel("PC1 Score", fontsize=14)
    ax_big.set_ylabel("Frequency", fontsize=14)
    ...
    for i, ax in enumerate(subaxes, start=1):
        if i < top5:
            ...
            ax.set_title(f"PC{i+1} ({explained_variance[i]*100:.1f}% var)", fontsize=14)
            ax.set_xlabel(f"PC{i+1} Score", fontsize=14)
...
def plot_ptm_importance(ptm_site_labels, ptm_matrix, system_name, outdir="figures_pca"):
    ...
    plt.xlabel("PTM Residue (resname-chainID-resid) [domain]", fontsize=14)
    plt.ylabel("Mean Residue Contacts", fontsize=14)
...
def plot_top5_pcs_all_systems(pca_data_dict, outdir="figures_pca", filename="all_systems_top5_PCs.pdf"):
    ...
    for i, sys_name in enumerate(systems):
        ...
        for j in range(n_pcs):
            ...
            ax.set_title(f"{sys_name} PC{j+1}\n({explained_variance[j]*100:.1f}% var)", fontsize=14)
            ax.set_xlabel(f"PC{j+1} Score", fontsize=14)
            ax.set_ylabel("Frequency", fontsize=14)
...
def plot_pairwise_top5_pcs(pca_data_dict, cluster_labels_dict=None, outdir="figures_pca", filename="pairwise_top5_PCs.pdf"):
    ...
    for sys_name, (pca_data, explained_variance) in pca_data_dict.items():
        ...
        for subplot_index, (i, j) in enumerate(pairs):
            ...
            ax.set_xlabel(f"PC{i+1} ({explained_variance[i]*100:.1f}% var)", fontsize=14)
            ax.set_ylabel(f"PC{j+1} ({explained_variance[j]*100:.1f}% var)", fontsize=14)
            ax.set_title(f"{sys_name}: PC{i+1} vs PC{j+1}", fontsize=14)
        ...
        ncol = len(handles)  
        fig.legend(handles, labels, loc='upper center', bbox_to_anchor=(0.5, 1.10), ncol=ncol,frameon=False, fontsize=14)   
        os.makedirs(outdir, exist_ok=True)
        outfile = os.path.join(outdir, filename)
