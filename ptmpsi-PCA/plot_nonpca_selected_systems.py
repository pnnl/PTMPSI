...
def plot_rmsf(base, systems, skip, mode, cutoff, topo, traj, blk, outdir):
    ...
    # 6) Y-axis labels
    ax0.set_yticks([0])
    ax0.set_yticklabels(["REF"], fontsize=14)
    yt = list(range(len(systems)-1))
    yl = [str(int(s)) for s in systems[1:]]
    ax1.set_yticks(yt)
    ax1.set_yticklabels(yl, fontsize=14)
    ax1.invert_yaxis()
    ...
    ax1.set_xticks(mids)
    ax1.set_xticklabels(bottom_labels, rotation=0, fontsize=14)
    ...
    ax0_top.set_xticklabels(labs, fontsize=14, rotation=0)
