...
def modify(session, residues, res_type, *, bfactor=None, clash_hbond_allowance=None,
        ...
        hbond_angle_slop=None, hbond_dist_slop=None, hbond_relax=True, ignore_other_models=False,
           rot_lib="CysPTM", log=True, preserve=None, retain=False):
    """backend implementation of "modify" command."""
    rotamers = {}
    destroy_list = []
    for res in residues:
        if res_type == "same":
...
def get_rotamers(session, res, phi=None, psi=None, cis=False, res_type=None, rot_lib="CysPTM", log=False):
    ...
    if res.name != "CYS":
        raise NoResidueRotamersError("No PTMs available for %s" % res_type)
    ...
    res_template_func = rot_lib.ptm_template_func
    ...
    template = rot_lib.ptm_template_func(session,mapped_res_type)
    ...
    for i, rp in enumerate(params):
        ...
        registerer = "do_ptm get_rotamers"
