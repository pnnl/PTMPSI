...
class PrepRotamersDialog(ToolInstance):
    ...
    def launch_rotamers(self):
        ...
        try:
            run(self.session, "modify interactive sel %s rotLib %s" % (res_type, lib_name))
        except NoResidueRotamersError:
            run(self.session, "modify sel %s rotLib %s" % (res_type, lib_name))

    def lib_res_list(self):
        res_name_list = list(self.rot_lib.residue_names)
        res_name_list.sort()
...
class RotamerDialog(ToolInstance):
    ...
    help = "help:user/tools/rotamers.html"
    SESSION_SAVE = True
    registerer = "do_ptm RotamerDialog"
    ...
    def finalize_init(self, mgr, res_type, rot_lib_name, *, table_info=None):
        ...
        if _settings is None:
            ...
            _settings = _RotamerSettings(self.session, "PTM")
        from chimerax.ui import MainToolWindow
        self.tool_window = tw = MainToolWindow(self)
        parent = tw.ui_area
    ...
    def _apply_rotamer(self):
        ...
        from chimerax.core.commands import run
        cmd = "modify %s %s criteria %s rotLib %s" % (
